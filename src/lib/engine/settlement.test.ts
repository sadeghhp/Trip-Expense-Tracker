...
describe('recalculateExchangeRates', () => {
  it('returns old rates when old and new settlement are the same', () => {
    ...
    const result = recalculateExchangeRates(rates, 'USD', 'USD');
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.reason).toBe('same_currency');
  });

  it('reports failure when pivot rate is missing', () => {
    const rates = { EUR: 0.9 };
    const result = recalculateExchangeRates(rates, 'USD', 'TRY');
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.reason).toBe('no_pivot_rate');
  });

  it('reports failure when old settlement is empty', () => {
    const result = recalculateExchangeRates({ EUR: 0.9 }, '', 'EUR');
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.reason).toBe('missing_currency');
  });

  it('reports failure when new settlement is empty', () => {
    const result = recalculateExchangeRates({ EUR: 0.9 }, 'USD', '');
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.reason).toBe('missing_currency');
  });

  it('recalculates cross-rates for two non-settlement currencies', () => {
    const result = recalculateExchangeRates({ CNY: 7.25, TRY: 32.5 }, 'USD', 'CNY');
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.rates['TRY']).toBeCloseTo(32.5 / 7.25, 6);
    expect(result.rates['USD']).toBeCloseTo(1 / 7.25, 6);
    expect(result.rates['CNY']).toBeUndefined();
  });

  it('adds rate for the old settlement currency', () => {
    const result = recalculateExchangeRates({ EUR: 2 }, 'USD', 'EUR');
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.rates['USD']).toBeCloseTo(0.5, 6);
  ...
  it('preserves conversion equivalence after switch', () => {
    ...
    const inCNY = amountTRY / result.rates['TRY'];
    const inUSD_viaCNY = inCNY * result.rates['USD'];

    expect(inUSD_viaCNY).toBeCloseTo(inUSD_direct, 6);
  });

  it('handles three non-settlement currencies', () => {
    const result = recalculateExchangeRates({ EUR: 0.92, GBP: 0.79, JPY: 157 }, 'USD', 'EUR');
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.rates['GBP']).toBeCloseTo(0.79 / 0.92, 6);
    expect(result.rates['JPY']).toBeCloseTo(157 / 0.92, 6);
    expect(result.rates['USD']).toBeCloseTo(1 / 0.92, 6);
    expect(result.rates['EUR']).toBeUndefined();
  });

  it('skips zero or negative rates in old rates', () => {
    const result = recalculateExchangeRates({ EUR: 0.9, TRY: 0, GBP: -1 }, 'USD', 'EUR');
    expect(result.ok).toBe(true);
    if (!result.ok) return;
    expect(result.rates['USD']).toBeCloseTo(1 / 0.9, 6);
    expect(result.rates['TRY']).toBeUndefined();
    expect(result.rates['GBP']).toBeUndefined();
  });

  it('reports failure when pivot rate is zero', () => {
    const result = recalculateExchangeRates({ EUR: 0 }, 'USD', 'EUR');
    expect(result.ok).toBe(false);
    if (!result.ok) expect(result.reason).toBe('no_pivot_rate');
  });

  it('round-trips back to original rates', () => {
    const switched = recalculateExchangeRates({ CNY: 7.25, TRY: 32.5 }, 'USD', 'CNY');
    expect(switched.ok).toBe(true);
    if (!switched.ok) return;
    const restored = recalculateExchangeRates(switched.rates, 'CNY', 'USD');
    expect(restored.ok).toBe(true);
    if (!restored.ok) return;

    expect(restored.rates['CNY']).toBeCloseTo(7.25, 6);
    expect(restored.rates['TRY']).toBeCloseTo(32.5, 6);
    expect(restored.rates['USD']).toBeUndefined();
  });
});
