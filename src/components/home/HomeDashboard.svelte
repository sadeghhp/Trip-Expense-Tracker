<script lang="ts">
  ...
  import {
    ...
    TrendingUp, Calendar, Receipt, Crown, Sparkles, ChevronLeft
  } from '@lucide/svelte';
  import { appData, activeTrip, exitTrip } from '$lib/stores/data';
  ...
  let totalSpending = $derived.by(() => {
    const { expenses, currencies, exchangeRates, settlementCurrency } = $appData;
    if (!expenses.length || !settlementCurrency) return 0;
    let total = 0;
    for (const exp of expenses) {
      if (exp.currencyCode === settlementCurrency) {
        total += exp.amount;
      } else {
        const rate = exchangeRates[exp.currencyCode];
        if (rate) total += exp.amount / rate;
      }
    }
  ...
  let settlementCurrencySymbol = $derived(
    getCurrencySymbol($appData.settlementCurrency, $appData.currencies)
  );

  let balances = $derived(computeBalances($appData.expenses));

  let unifiedBalances = $derived(
    computeUnifiedBalances(balances, $appData.participants, $appData.settlementCurrency, $appData.exchangeRates)
  );
  ...
  let todayTotal = $derived.by(() => {
    const { exchangeRates, settlementCurrency } = $appData;
    let total = 0;
  ...
  let biggestSpender = $derived.by(() => {
    const { expenses, participants, exchangeRates, settlementCurrency } = $appData;
    if (!expenses.length || !participants.length) return null;
    const spending: Record<string, number> = {};
    for (const exp of expenses) {
      const rate = exp.currencyCode === settlementCurrency ? 1 : (1 / (exchangeRates[exp.currencyCode] || 1));
...
<div class="px-4 md:px-6 py-4 md:py-6 space-y-5">
  ...
  <div class="flex items-center gap-3 -mt-1 mb-1">
    ...
    >
      <ChevronLeft size={20} class="text-[var(--text-secondary)]" />
    </button>
    <div class="flex-1 min-w-0">
      <p class="text-xs text-[var(--text-secondary)] font-medium">Trip Dashboard</p>
    </div>
  </div>
  ...
  >
    ...
    <div class="relative z-10">
      <div class="flex items-start justify-between mb-4">
        <div>
          <h1 class="text-xl md:text-2xl font-bold tracking-tight">{$activeTrip?.name ?? 'Trip'}</h1>
          <p class="text-white/70 text-sm mt-0.5 flex items-center gap-1.5">
            <Calendar size={13} />
            <span>{tripDuration}</span>
            <span class="mx-1">·</span>
            <Users size={13} />
            <span>{$appData.participants.length} {$appData.participants.length === 1 ? 'person' : 'people'}</span>
          </p>
        </div>
        {#if $appData.settlementCurrency}
          <span class="text-xs bg-white/15 backdrop-blur-sm px-2.5 py-1 rounded-full font-medium">
      ...
      <div class="mt-5">
        <p class="text-white/60 text-xs font-medium uppercase tracking-wider">Total Spending</p>
      ...
      <div class="flex gap-4 mt-5 pt-4 border-t border-white/15">
        <div>
          <p class="text-white/60 text-[11px] uppercase tracking-wider">Expenses</p>
          <p class="text-lg font-semibold">{$appData.expenses.length}</p>
        </div>
        <div>
          <p class="text-white/60 text-[11px] uppercase tracking-wider">Today</p>
          <p class="text-lg font-semibold">{settlementCurrencySymbol}{formatAmount(todayTotal)}</p>
        </div>
        <div>
          <p class="text-white/60 text-[11px] uppercase tracking-wider">Avg/Day</p>
  ...
  <section in:fly={{ y: 20, duration: 300, delay: 100 }}>
    <div class="grid grid-cols-3 md:grid-cols-6 gap-2">
      ...
      >
        ...
        <span class="text-[11px] font-medium text-primary-700 dark:text-primary-300">Expense</span>
      </button>
      ...
      >
        ...
        <span class="text-[11px] font-medium text-[var(--text-secondary)]">People</span>
      </button>
      ...
      >
        ...
        <span class="text-[11px] font-medium text-[var(--text-secondary)]">Currency</span>
      </button>
      ...
      >
        ...
        <span class="text-[11px] font-medium text-[var(--text-secondary)]">Balances</span>
      </button>
      ...
      >
        ...
        <span class="text-[11px] font-medium text-[var(--text-secondary)]">Settle</span>
      </button>
      ...
      >
        ...
        <span class="text-[11px] font-medium text-[var(--text-secondary)]">Export</span>
      </button>
    </div>
  </section>
  ...
  {#if $appData.participants.length > 0 && $appData.expenses.length > 0}
    ...
    >
      <div class="flex items-center justify-between mb-3">
        <h2 class="text-sm font-semibold text-[var(--text-primary)]">Balances</h2>
        <button
          onclick={() => onNavigate('balances')}
  ...
  {#if settlements.length > 0}
    ...
    >
      <div class="flex items-center justify-between mb-3">
        <h2 class="text-sm font-semibold text-[var(--text-primary)]">Settlement</h2>
        <button
          onclick={() => onNavigate('settlement')}
          class="text-xs text-primary-600 dark:text-primary-400 font-medium hover:underline"
        >
      ...
      <div class="flex gap-4 text-center">
        <div class="flex-1 py-2 rounded-xl bg-surface-50 dark:bg-surface-800/50">
          ...
          <p class="text-[11px] text-[var(--text-secondary)]">Transfers needed</p>
        </div>
        <div class="flex-1 py-2 rounded-xl bg-surface-50 dark:bg-surface-800/50">
          <p class="text-lg font-bold text-[var(--text-primary)]">{debtors.length}</p>
          <p class="text-[11px] text-[var(--text-secondary)]">Owe money</p>
        </div>
        <div class="flex-1 py-2 rounded-xl bg-surface-50 dark:bg-surface-800/50">
          <p class="text-lg font-bold text-[var(--text-primary)]">{creditors.length}</p>
          <p class="text-[11px] text-[var(--text-secondary)]">Are owed</p>
        </div>
      </div>
    </section>
  {/if}
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-[11px] text-[var(--text-secondary)] mt-0.5">
                ...
                · {expense.beneficiaries.length} {expense.beneficiaries.length === 1 ? 'person' : 'people'}
              </p>
            </div>
  ...
  {#if $appData.expenses.length >= 3}
    ...
    >
      <div class="flex items-center gap-2 mb-3">
        ...
        <h2 class="text-sm font-semibold text-[var(--text-primary)]">Insights</h2>
      </div>
      ...
      <div class="grid grid-cols-2 gap-3">
        {#if biggestSpender}
          <div class="p-3 rounded-xl bg-surface-50 dark:bg-surface-800/50">
            <div class="flex items-center gap-1.5 mb-1">
              ...
              <p class="text-[11px] text-[var(--text-secondary)] font-medium">Top Spender</p>
            </div>
        ...
        <div class="p-3 rounded-xl bg-surface-50 dark:bg-surface-800/50">
          <div class="flex items-center gap-1.5 mb-1">
            ...
            <p class="text-[11px] text-[var(--text-secondary)] font-medium">Total Expenses</p>
          </div>
        ...
        <div class="p-3 rounded-xl bg-surface-50 dark:bg-surface-800/50">
          <div class="flex items-center gap-1.5 mb-1">
            ...
            <p class="text-[11px] text-[var(--text-secondary)] font-medium">Daily Avg</p>
          </div>
        ...
        <div class="p-3 rounded-xl bg-surface-50 dark:bg-surface-800/50">
          <div class="flex items-center gap-1.5 mb-1">
            ...
            <p class="text-[11px] text-[var(--text-secondary)] font-medium">Today</p>
          </div>
          <p class="text-sm font-semibold text-[var(--text-primary)]">
  ...
  {#if $appData.expenses.length === 0}
    ...
    >
      ...
      <h3 class="text-base font-semibold text-[var(--text-primary)] mb-1">No expenses yet</h3>
      <p class="text-sm text-[var(--text-secondary)] mb-5 max-w-xs mx-auto">
        Start tracking your trip expenses. Add participants and currencies first, then record your spending.
      </p>
...
{#if $appData.participants.length > 0 && $appData.currencies.length > 0}
  <button
    onclick={() => showExpenseForm = true}
    class="fixed bottom-20 md:bottom-8 right-5 md:right-8 z-40 w-14 h-14 rounded-2xl bg-gradient-to-br from-primary-500 to-primary-700 text-white shadow-[0_8px_30px_var(--fab-shadow)] hover:shadow-[0_12px_40px_var(--fab-shadow)] hover:scale-105 active:scale-95 transition-all duration-200 flex items-center justify-center"
