<script lang="ts">
  import { Plus, Pencil, Trash2, Receipt, AlertTriangle, ArrowRightLeft, BookOpen } from '@lucide/svelte';
  ...
  import JournalList from '../journals/JournalList.svelte';
  ...
  let highlightExpenseId: string | null = $state(null);
  ...
  function journalLabelForExpense(expense: Expense): string | null {
    if (!expense.journalEntryId) return null;
    const journal = $appData.journals.find(j => j.id === expense.journalEntryId);
    return journal?.journalId ?? null;
  }
  ...
  function handleViewExpense(expenseId: string) {
    ...
    setTimeout(() => { highlightExpenseId = null; }, 2000);
  }
</script>
...
<div class="p-4 md:p-6 space-y-4">
  ...
  <JournalList onViewExpense={handleViewExpense} initialExpanded={journalsExpanded} />
    ...
    <div class="space-y-3">
      {#each sortedExpenses as expense, i (expense.id)}
        <div
          ...
          class="relative p-4 ps-9 bg-[var(--card-bg)] border rounded-2xl shadow-sm hover:shadow-md transition-all duration-200
            ...
            <div class="flex items-start gap-3 flex-1 min-w-0">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 mb-1 flex-wrap">
                  ...
                  {#if journalLabelForExpense(expense)}
                    <span class="shrink-0 flex items-center gap-1 px-2 py-0.5 rounded-lg bg-[var(--app-bg)] border border-[var(--card-border)] text-[10px] font-medium text-[var(--text-secondary)]">
                      <BookOpen size={10} />
                      {$t('journals.fromJournal', { id: journalLabelForExpense(expense) })}
                    </span>
                  {/if}
                </div>
