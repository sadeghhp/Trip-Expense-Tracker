  ...
  {:else}
    <div class="space-y-3">
      {#each sortedExpenses as expense (expense.id)}
        <div class="p-4 bg-[var(--card-bg)] border border-[var(--card-border)] rounded-2xl shadow-sm">
          <div class="flex items-start justify-between">
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-1">
                <span class="text-sm font-semibold text-[var(--text-primary)] truncate">{expense.description}</span>
                <span class="shrink-0 px-2 py-0.5 rounded-lg bg-surface-100 dark:bg-surface-800 text-[10px] font-bold text-[var(--text-secondary)]">
            ...
            <div class="flex items-center gap-2 ml-3">
              ...
              <div class="flex gap-0.5">
                <button
                  ...
                  class="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-surface-100 dark:hover:bg-surface-800 transition-colors"
                >
                  <Pencil size={14} class="text-[var(--text-secondary)]" />
                </button>
                <button
                  onclick={() => requestDelete(expense)}
                  class="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-danger-500/10 transition-colors"
                >
          ...
          <div class="mt-2 flex items-center gap-1 flex-wrap">
            <span class="text-[10px] text-[var(--text-secondary)]">Split ({expense.splitType}):</span>
